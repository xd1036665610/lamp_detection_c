...
int main() 
{
	...
	for (int imagenameNum = 0; imagenameNum < 31; imagenameNum++) {
		...
		string referImage = "1(" + intTostring(imagenameNum)+").jpg";
			...
			if ((tmplatepoints[0].x - (widratio - 1) / 2 *roiwidth > 0) && (tmplatepoints[0].x + widratio*roiwidth < perspectiveImage.cols))
			{
				...
				if (upRow > 0 && downRow < roImage.rows&&upRow < downRow) {
					...
					if (rowNonSignal.size() > 0) {
						...
						{
							Mat dst;
							judgelightORblack(roImage, dst, signalRow);
							imshow("dst", dst);
...
void tmplateMatch(Mat src, Mat &roImage, vector<int>&rowNonSignal, int upRow, int downRow) {
	...
	if (downRow<grayroImage.rows&&upRow>0&&downRow>upRow)
	{
		...
		if (tmplateScore.size() > 2 * borden) {
			...
			//}
			//中间
			for (int i = borden; i < tmplateScore.size() - borden; i++) {
				int judge = 0;
				//int bordenNum = 0;
				for (int j = 1; j < borden; j++) {
			...
			/*int rowNonSize = rowNonSignal.size();
			for (int i = 0; i < rowNonSize; ) {
				double mean=0;
				for (int j = 0; j < 15; j++) {
					mean += ((tmplateScore[rowNonSignal[i] + j] - tmplateScore[rowNonSignal[i]]) +
						(tmplateScore[rowNonSignal[i] + j] - tmplateScore[rowNonSignal[i]])) / 30;
				}

				if (mean < 0.02) {
					swap(rowNonSignal[i], rowNonSignal[rowNonSize - 1]);
			...
			sort(rowNonSignal.begin(), rowNonSignal.end());*/
			//后边界
			//for (int i = tmplateScore.size() - borden ; i < tmplateScore.size() ; i++) {
			//	int judge = 0;
			//	//int tmp = i;
			//	//int bordenNum = 0;
			//	for (int j = 1; j < borden; j++) {
